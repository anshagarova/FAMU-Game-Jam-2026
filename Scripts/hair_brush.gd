...
@export var strand_length: float = 18.0      
@export var strand_wobble: float = 0.35   
...
var _stroke_dir: Vector2 = Vector2(1, 0)    
...
func _ready() -> void:
	...
	if palette != null:
		...
		print("HairBrush: connected to palette")
	else:
		push_warning("HairBrush: no palette assigned in Inspector")
...
func _stamp(center: Vector2):
	for i in bristle_count:
		...
		var root = Vector2(center.x + cos(angle) * dist, center.y + sin(angle) * dist)

		var wobble = _rng.randf_range(-strand_wobble*0.5, strand_wobble*0.5)
		var strand_dir = _stroke_dir.rotated(wobble)

		var length_factor = 1.0 - rim_t * 0.3
		var length = strand_length * length_factor * _rng.randf_range(0.8, 1.0) * (brush_radius / 10.0)

		var tip = root + strand_dir * length
		_draw_strand(root, tip, alpha)

	_texture.update(_canvas)

func _draw_strand(a: Vector2, b: Vector2, alpha: float):
	var steps = max(int((b - a).length()), 1)
	for s in steps:
		var frac = float(s) / float(steps)
		var p = a.lerp(b, frac)

		var tip_fade = pow(1.0 - frac, 1.5)
		_paint_dot(int(p.x), int(p.y), 1, alpha * tip_fade)
